...
public class UI : MonoBehaviour
{
    ...
    public void OnClick(string click)
    {
        switch (click)
        {
            ...
            case "Diagnal1":
                sfx.PlaySound(0, 0);
                //board.currentPlayer.MoveDiagonally(true);
                break;
            case "Diagnal2":
                sfx.PlaySound(0, 0);
                //board.currentPlayer.MoveDiagonally(false);
                break;
    ...
    public void OpenWinPanel()
    {
        ...
        LeanTween.value(WinPanelBg.gameObject, (float val) => { WinPanelBg.color = new Color(before.r, before.g, before.b, val); }, 0, .945f, 1f).setOnComplete(() => { AdsManager.Instance?.ShowInterstitialAd(); });
        sfx.PlaySound(10, 0);
    }
