...
public class Player : MonoBehaviour
{
                ...
                if (board.spacesToMove != 0)
                {
                    ...
                    LeanTween.move(gameObject, piecePos, .4f).setEaseInOutCubic().setOnComplete(() =>
                    {
                        ...
                        if (board.spacesToMove == 0) // When done moving
                        {
                            ...
                            if (piece.Recource != CardScript.Resource.None && !(piece.Recource == CardScript.Resource.Shelter && isKidPlayer))
                            {
                                sfx.PlaySound(3,0);
    ...
    int index;
    int counter;
    int savedSpacesToMove;
    private bool hasStoppedAtCheckpoint = false; // Flag to track stopping at a checkpoint

    public void MoveDirection(bool clockWise)
    {
        if (hasStoppedAtCheckpoint)
        {
            Debug.Log("Continuing movement without stopping at checkpoints.");
            hasStoppedAtCheckpoint = false; // Reset the flag for the next full movement cycle
        }

        index = board.clockWise.IndexOf(PlayerIsOn);
        counter = index;
        savedSpacesToMove = board.spacesToMove;
        ...
        for (int i = 0; i < savedSpacesToMove; i++)
        {
            ...
            // Move the player to the next piece
            MovePlayer(board.clockWise[counter]);

            // Check if the counter is one of the checkpoint indices
            if (!hasStoppedAtCheckpoint && (counter == 15 || counter == 10 || counter == 5 || counter == 0))
            {
                Debug.Log($"Reached checkpoint at {counter}. Please choose a direction to continue.");
                //board.spacesToMove -= (i + 1); // Subtract spaces already moved
                ui.SetMoveText(board.spacesToMove);
                hasStoppedAtCheckpoint = true; // Set the flag
                return; // Exit the function to wait for further direction
            }
        }

        // Reset the flag after completing the movement
        hasStoppedAtCheckpoint = false;
    }
